...
local outfitName = string.lower(outfits[choice.id].name) 
-- Converts the outfit name to lowercase    
...
for i = 1, #outfits do    local o = outfits[i].name           
...
-- Set what button is pressed when the player presses enter or escape    
window:setDefaultEnterButton("Back")    window:setDefaultEscapeButton("Back")     
-- Send the window to player    
window:sendToPlayer(self)
end 
