...
{
	// editado era metade
	return static_cast<int32_t>(std::ceil((attackSkill * (attackValue * 50.)) + (attackValue * 50.) * 10));
}
...
{
	// editado era 4 e 3
	return static_cast<int32_t>(std::round((level / 5) + (((((attackSkill / 3.) + 1) * (attackValue / 2.)) * 1.03) * 10 / attackFactor)));
}
